...
| --- | --- | --- |
| `Clock` | 補充間隔(24h)判定、時計表示、おやすみタイマー | システム時刻 / 固定・手動進行の fake |
...
| おやすみタイマー | 設定分後に暗転フラグが立つ(fake clock + fake_async)/ OFF で立たない / タップで解除 |
...
| カスタマイズ(TUNE) | テーマチップ選択で対象カテゴリに切替 / 各トグル・セグメントが設定に反映 / 間隔・おやすみの選択 |
